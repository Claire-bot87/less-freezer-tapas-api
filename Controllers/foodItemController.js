...
// * Index route
router.get('/foodItems', async (req, res, next) => {
  try {
...
// * Create route
router.post('/foodItems', validateToken, async (req, res, next) => {
  try {
...
// * Show route
router.get('/foodItems/:foodItemId', async (req, res, next) => {
  try {
    const { foodItemId } = req.params

    // 1. Search for the post based on the postId in the params
    const foodItem = await FoodItem.findById(foodItemId).populate('type')
